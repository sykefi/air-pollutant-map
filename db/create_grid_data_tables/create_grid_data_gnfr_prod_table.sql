...
CREATE TABLE public.grid_data_gnfr_prod_groups AS (
    SELECT vuosi, grid_id, gnfr_prod, sum(s16) as s16,sum(s15) as s15,sum(s22) as s22,sum(s13) as s13,sum(s28) as s28,sum(s29) as s29,sum(s27) as s27,sum(s43) as s43,sum(s5) as s5,sum(s18) as s18,sum(s3) as s3,sum(s12) as s12,sum(s1) as s1,sum(s7) as s7,sum(s8) as s8,sum(s14) as s14,sum(s37) as s37,sum(s25) as s25,sum(s19) as s19,sum(s17) as s17,sum(s38) as s38,sum(s40) as s40
...
-- select vuosi, grid_id, gnfr_prod, s16 from public.grid_data_gnfr_prod_groups ORDER BY grid_id limit 30;
...
CREATE TABLE grid_data_gnfr_prod AS (
    SELECT vuosi, grid_id, geom_web as geom, gnfr_prod as gnfr, s16,s15,s22,s13,s28,s29,s27,s43,s5,s18,s3,s12,s1,s7,s8,s14,s37,s25,s19,s17,s38,s40
    FROM grid_data_gnfr_prod_groups AS GD
    INNER JOIN emep_grid_01 AS G ON G.id = GD.grid_id
);
